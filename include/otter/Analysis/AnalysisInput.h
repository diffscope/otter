...
namespace otter {
    ...
    ///
    /// \a knob is what the module declared. A knob the module does not honor ignores \a given
    /// and yields \a fallback, the implementation's own constant, because the declaration said
    /// the caller has no say. An honored knob yields its declared default when \a given is empty.
    /// A value outside the declared range is refused rather than clamped: a caller that asked for
    /// a setting the module cannot honor should learn that, not receive a different answer under
    /// the name of the one it asked for.
    /// \{
